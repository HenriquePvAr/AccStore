...
const roleItems: Record<UserRole, SidebarItem[]> = {
  customer: [
    ...
    { label: 'Minhas propostas', view: 'myProposals', icon: FileSearch },
...
export function Sidebar({ activeView, onNavigate }: SidebarProps) {
  const { user } = useAuth()
  const sidebarItems = user ? roleItems[user.role] : publicItems
      ...
      <div className="mt-auto border-t border-[rgba(120,140,255,0.16)] pt-3">
        ...
        >
          ...
          <span className="min-w-0 leading-tight">
            <span className="block truncate text-[13px] font-semibold text-white">{user?.name ?? 'Entrar'}</span>
            <span className="block truncate text-xs font-medium text-slate-500">{user ? roleLabels[user.role] : 'Entre para continuar'}</span>
          </span>
        </button>
      </div>
    </aside>
