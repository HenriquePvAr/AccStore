...
begin
  alter table public.conversations
    add constraint conversations_proposal_id_fkey
    foreign key (proposal_id) references public.sell_proposals(id) on delete set null;
exception
...
where role in ('seller', 'admin') or verified = true;
...
create index if not exists orders_payment_provider_id_idx on public.orders (payment_provider, payment_provider_id) where payment_provider_id is not null;
create index if not exists payment_events_order_id_idx on public.payment_events (order_id);
create index if not exists payment_events_payment_id_idx on public.payment_events (provider, payment_id) where payment_id is not null;
...
with check (
  exists (
    ...
    from public.accounts a
    where a.id = account_id
      and (public.is_account_owner(a.id) or public.is_admin())
  )
);
...
using (
  exists (
    ...
    where c.id = conversation_id
      and (public.is_conversation_participant(c.id) or public.is_admin())
  )
);
